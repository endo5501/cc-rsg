...
5. **depth モード判定（規模ベース）**
   - 偵察時に把握した**ファイル総数**を `recon-report.md` の冒頭に記録する。`.cc-rsg/state.json` に `total_files` として保存。
   - **200 ファイル超**の場合は、利用者に AskUserQuestion で **depth モード**を選択させる:
     - `comprehensive`: 現状の動作。全章詳細・MECE 完全・全 REF。**監査・規制対応など完全網羅が必要な場合のみ推奨**（数時間〜数日のコストを要する）
     - `outline`（**推奨デフォルト**）: 各レベルの entity を**表で全列挙** + Mermaid 図 + 各表末尾に「深掘り候補」リスト。詳細は Phase 6 後の対話で on-demand 生成。**通常用途に最適**
     - `interactive`: outline モードと同じ流れ + Phase 6 完了後も継続的に深掘り受付。**チームで継続参照する場合**
   - **200 ファイル以下**の場合は `comprehensive` をデフォルトとして自動選択（質問なし）。ただし利用者が明示的に変更を希望すれば受け付ける。
   - 選択結果を `.cc-rsg/goal.json` に `depth_mode: "comprehensive" | "outline" | "interactive"` として保存。以降の Phase 2 / 3 / 4 / 6 はこの値で分岐する。
   - 質問文の例:
     > 対象コードベースは N ファイル / X 行と大規模です。仕様書の深度モードを選んでください。
     > （概観のみ → 気になる箇所を後で深掘り、という運用が現実的に推奨です）
...
5. **depth モードによる章構成の調整（Phase 1 で確定したモード）**

   `.cc-rsg/goal.json` の `depth_mode` 値で WBS の章構成を分岐する:

   **(a) `comprehensive`（従来動作・監査用途）**
   - 13章ベースのテンプレート章立てに `assigned_inventory_ids` を分散させる
   - 各章 200 行 / REF 10 件以上を Phase 3 で生成
   - **大規模リポでは数時間〜数日を要する。読者は監査担当が前提**

   **(b) `outline` または `interactive`（推奨デフォルト）**
   - **章は「表中心」に再構成**する。`references/outline-tables.md` のカタログを参照して言語別に決める
   - **必須章（Layer 1: MECE 保証）**:
     - `01-modules-overview.md` — Modules 表（責務区分の全列挙）
     - `02-entities.md` — Entities 表（model / struct / type / class / interface の全列挙）
     - `03-actions.md` — Actions 表（controller / handler / endpoint の全列挙）
     - `04-data.md` — Data 表（DB schema / migrations）
     - `05-dependencies.md` — Dependencies 表（gem / pip / npm 等）
   - **必須章（Layer 2: 関係性可視化）**:
     - `06-diagrams.md` — Mermaid（ER 図、モジュール依存、代表シーケンス、状態遷移）
   - **任意章（必要に応じて）**:
     - `07-flows.md` — ユースケース別シーケンス（複数）
     - `08-cross-cutting.md` — 認証 / ロギング / トランザクション等の横断関心事
   - **予約章**は変わらず `00-metadata.md` / `99-unresolved.md` / `traceability.md`
   - **本文 200 行 / REF 10 件は強制しない**。代わりに各表が**全 entity を列挙**していることを `coverage-check.py` で検査する
   - `wbs.json` の各 chapter にも `depth_mode` を記録し、Phase 3 / 4 で参照可能にする

6. **利用者レビュー**
   - WBSとスケルトンを表示し、利用者に「この分割でPhase 3を開始してよいか」を確認する。
   - outline / interactive モードでは「概観テーブル中心の章立て」になっていることも明示し、納得を得る。
...
`.cc-rsg/goal.json` の `depth_mode` で **Phase 3 全体の動作モードが変わる**:

| depth_mode | 主な動作 | 章本文の形 |
|---|---|---|
| `comprehensive` | 以下の STEP A〜F を全章に適用 | 200 行 + REF 10 件 + Mermaid 1 個 + 5 Sources Read 以上の長文章 |
| **`outline` / `interactive`** | **STEP A〜F を Layer 1 / 2 章に置き換え**（次の「outline モードの章執筆」節を参照）| 表中心・関係性図 + 深掘り候補リスト |

`outline` / `interactive` モードでは以下の `comprehensive` 用 STEP は適用されない:
- 「200 行以上」の本文行数強制
- 「REF 10 件以上」の引用件数強制
- 「Sources Read 5 件以上」の必須数

代わりに **outline モードの執筆ルール**（後述）に従う。

---

### 必須原則（厳守）— `comprehensive` モード時
...
### outline モードの章執筆（`depth_mode == "outline" | "interactive"` 時）

このセクションは **`comprehensive` モードでは適用されない**。outline モードの場合、Phase 3 の動作は以下の OUT-A〜OUT-D に置き換わる。

#### OUT-A: Layer 1 章（02-entities / 03-actions / 04-data / 05-dependencies）の生成

各 Layer 1 章は **「対応言語の概観テーブル」を全列挙**する。手順:

1. `.cc-rsg/skill/references/outline-tables.md` で言語別カタログを参照
2. **`glob` + `grep` で全 entity を機械抽出**:
   - Ruby/Rails の Models: `grep "^class \\w+" --type ruby app/models/`
   - Controllers: `grep "^class \\w+Controller" --type ruby app/controllers/`
   - 等、言語に応じて outline-tables.md のパターン使用
3. 抽出結果を **Markdown 表として全列挙**。漏らさない。1 entity = 1 行
4. 各セルに **Confidence ラベル**（🟢 VERIFIED / 🟡 INFERRED / 🔴 ASSUMED）を必ず付与
   - 🟢: その entity のファイルを `file_editor view` で確認した
   - 🟡: `grep` ヒットのみ確認、本文は未読
   - 🔴: フレームワーク典型挙動からの推測
5. 概要列は 1 行（80 文字以内）。**詳細ロジックは書かない** — 深掘りは Layer 3 で

**章末に必ず「深掘り候補」セクション**を置く（OUT-C 参照）。

#### OUT-B: Layer 2 章（06-diagrams）の Mermaid 生成

- ER 図（Entities + Data 表から自動派生）
- モジュール依存図
- 代表シーケンス（最も典型的なリクエストフロー 1〜3 本）
- 状態遷移図（主要 entity の status カラム等が存在する場合）

各図には**1 行のキャプション**と「読み方の補助」を添える。図の出典セルが `[INFERRED]` の場合はその旨を明示。

#### OUT-C: 各 Layer 1 章末の「深掘り候補」リスト
...
- **D-003**: シーケンス「Issue 通知配信」の subscribers 解決 [🔴 ASSUMED]
```

選定基準（references/outline-tables.md の末尾を参照）:
1. 🔴 ASSUMED ラベルが多い行
2. 複雑度が高い行（メソッド数 / 関連数 / ファイル行数の上位 10%）
3. business-critical キーワード（auth / payment / permission / audit 等）を含む行

#### OUT-D: 章本文の長さ制約は撤廃

outline モードでは:
- **200 行以上 / REF 10 件以上 / Sources Read 5 件以上 は適用しない**
- 代わりに「**全 entity が表のいずれかの行に列挙されているか**」が MECE 基準（Phase 4 の coverage-check.py が自動判定）
- 章本文は表 + 1〜2 段落の解説 + Mermaid 図（該当章のみ）+ 深掘り候補リスト、で構成

---

### このフェーズで気を付けること
- **`comprehensive` モード時**: コードを読まずに章を書くのは禁止。Sources Read に列挙したファイルしか参照してはいけない。200 行 / REF 10 件 / 5 Sources Read を満たす必要あり。
- **`outline` / `interactive` モード時**: 「全 entity 列挙」を最優先。各セルの Confidence ラベルを正直に付ける。🟢 を過剰に付けない（実際に view したものだけ）。
...
## Phase 6.5: 対話深掘り受付モード（`depth_mode` が `outline` または `interactive` の時）

### 目的

`outline` / `interactive` モードでは Phase 6 完了時点の仕様書は「概観テーブル + Mermaid + 深掘り候補」のみ。**ユーザーが気になる箇所を読みながら指摘して、その場で詳細化していく**のがこのモードの本旨。Phase 6.5 は env が明示的に終了されるまで、agent が**深掘り受付状態で待機**する。

### 動作

Phase 6 完了報告の後、agent は以下のメッセージを出して**入力待ち**に入る:

```
✅ 概観仕様書の生成が完了しました（X 章 / Y 表 / Z 深掘り候補）。

各章末尾の「深掘り候補」セクションをご覧ください。気になる箇所は以下のように指示してください:

- 候補 ID で指示:   「D-001 を深掘りして」「D-007」
- entity ID で指示: 「M-013 Issue を詳しく」「C-018 ProjectsController」
- 自然文で指示:     「権限まわりを詳しく説明して」「Issue の通知配信の仕組みを」

deep-dive を終了する場合は「終了」「完了」「これで OK」と返信してください。
```
...
1. **明示 ID（最優先）**: `D-NNN` / `M-NNN` / `C-NNN` / `T-NNN` 等のパターンに一致 → 該当行/候補を `wbs.json` / `inventory.json` / 各章の表から検索 → 対応する file と概要を取得
2. **entity 名直接指定**: `Issue クラス` / `ProjectsController` 等の生クラス名 → grep で実ファイル特定
3. **自然文の topic 指示**: `権限` / `通知` / `決済` 等のキーワード → 該当章/表の行を keyword 検索し、上位 3 件をユーザーに提示して「どれを深掘りますか」と確認

### 深掘り章の生成

該当の深掘り対象が確定したら:

1. `chapter-investigator` サブエージェントを `task` ツールで起動
2. サブエージェントへの prompt:
   - 対象 entity / 候補の ID と概要
   - 該当する実コードファイル一覧
   - 「**comprehensive モード相当の品質基準**で 1 章を執筆せよ」（200 行+, REF 10+, Mermaid 1+, Sources Read 5+）
   - 出力先: `.cc-rsg/drafts/deep/D-NNN-{slug}.md` または `M-NNN-{slug}.md`
3. サブエージェントから戻った key findings をメインが表示
4. **traceability.md を更新**（深掘り章を追記）
5. **元の Layer 1 章の該当行を更新**: confidence を 🟡/🔴 → 🟢 に昇格、「→ 深掘り `D-001` 参照」リンクを追加
6. 完了報告し、再び入力待ちに戻る

### 終了

ユーザーが「終了」「完了」「これで OK」などの完了ワードを送信した時点で:

1. `state.json` を `phase_6_5_completed_at` で更新
2. `final/` 配下を再生成（深掘り章も含めて統合）
3. final/traceability.md を最終版に更新
4. お礼メッセージで env を closing

### このフェーズで気を付けること

- **ユーザー入力を待つ間、agent は polling / 自走を行わない**。明示的指示が来てから動く
- 同じ深掘り対象を**重複指示された場合**は、既存深掘り章を提示して「再生成しますか？」と確認
- 深掘り章を書く間も `chapter-investigator` の戻り値は **mode B 契約**（path + 要約）に従う
- 累積コストを意識: 深掘り 1 件あたり comprehensive 章 1 つ分のコスト。ユーザーに「現在 N 件深掘り完了、累積コスト ~$X」を定期的に示す
...
- v0.3.0 (2026-06-08): 大規模コードベース対応として **depth モード** (`comprehensive` / `outline` / `interactive`) を導入。Phase 1 末尾でファイル総数に応じて利用者にモード選択を促し、Phase 2/3/4/6 がモードで分岐する。`outline` モードでは Layer 1 章 (Modules / Entities / Actions / Data / Dependencies の概観テーブル) と Layer 2 章 (Mermaid 図) を生成し、各章末尾に「深掘り候補」リストを置く。`interactive` モードは加えて **Phase 6.5: 対話深掘り受付モード** で `chapter-investigator` を on-demand 起動し comprehensive 品質で部分章を生成する。各表セルに **Confidence ラベル** (🟢 VERIFIED / 🟡 INFERRED / 🔴 ASSUMED) を必須化。新規 `references/outline-tables.md` (341行、6言語 — Ruby/Rails、Python/Django、JS/TS/React、Go、Java/Kotlin Spring Boot — の概観テーブル定義 + Mermaid テンプレ + 深掘り選定基準) を追加。`coverage-check.py` を outline モード対応に拡張 (全 entity 列挙チェック / Confidence 比率検査)。
